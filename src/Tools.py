...
def generateOUprocess(T=10000.0, tau=3.0, mu=0.0, sigma=1.0, dt=0.1):
    ...
    def OU(T,tau,mu,sigma,dt):
            
            T_ind = int(T/dt)
    
            white_noise = np.random.randn(T_ind)
            white_noise = np.float32(white_noise)    
            
            OU_process = np.zeros(T_ind)
            OU_process = np.float32(OU_process)
            

            cT_ind  = np.int32(T_ind) 
            cdt     = np.float32(dt)
            ctau    = np.float32(tau)
            cmu     = np.float32(mu)            
            csigma  = np.float32(sigma) 
            OU_k1 = cdt / ctau 
            OU_k2 = np.sqrt(2.0*cdt/ctau) 
            for t in range(cT_ind-1) :
                OU_process[t+1] = OU_process[t] + (cmu - OU_process[t])*OU_k1 +  csigma*OU_k2*white_noise[t] 
            return OU_process
    OU_process = OU(T,tau,mu,sigma,dt)        
